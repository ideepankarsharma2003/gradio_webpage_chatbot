...
def generate_context(query:str, num_urls:int, is_content):
    ...
    if response.ok:
        ...
        _, chatbot= render_first()
        is_content= f"YES 😁, buffer content: {query}" 
        return chatbot, is_content
    else:
...
with gr.Blocks(server_name= '0.0.0.0') as demo:       
    ...
    with gr.Row():
        ...
        with gr.Column(scale=0.2):
            # num_urls = gr.Textbox(placeholder='# Urls', show_label=False, interactive=True).style(container=False)
            is_context = gr.Textbox(label=f"Context Generated for query: {search_url.value}", value="NO content in buffer🥲", show_label=True, interactive=True).style(container=False)
    ...
    gen_btn.click(
        fn=generate_context, 
        inputs=[search_url, num_urls, is_context],
        outputs=[chatbot_gen, is_context]
        )
